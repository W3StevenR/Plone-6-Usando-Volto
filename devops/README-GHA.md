...
2. Go to your repository at [collective/experimental](https://github.com/collective/experimental).
3. Click on `Settings` at the top-right corner.
4. In the left sidebar, click on `Environments`.
5. Press `New environment`.
6. Name the environment `experimental.example.com` and hit `Configure environment`.
...
|-------------|--------------|-------------|
| DEPLOY_HOST | experimental.example.com | The hostname or IP address of your Docker Swarm manager. |
...
| Name     | Value |
|----------|-------|
| LIVE_ENV | experimental.example.com |
...
- [Backend Tests Workflow](https://github.com/collective/experimental/actions/workflows/backend.yml)
- [Frontend Tests Workflow](https://github.com/collective/experimental/actions/workflows/frontend.yml)

Upon successful completion of the tests, Docker images for the Backend (`ghcr.io/collective/experimental-backend`) and Frontend (`ghcr.io/collective/experimental-frontend`) will be available.

### Initiating Manual Deployment

1. Navigate to [Manual Deployment of experimental.example.com](https://github.com/collective/experimental/actions/workflows/manual-deploy.yml).
...
- Initiate a new deployment using the `devops/stacks/experimental.example.com.yml` stack.
