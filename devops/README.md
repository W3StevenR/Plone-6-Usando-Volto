...
DEPLOY_ENV=prod
DEPLOY_HOST=experimental.example.com
DEPLOY_PORT=22
DEPLOY_USER=plone
...
You need either a Ubuntu or Debian based system for experimental.example.com, enable SSH, and install a supported version of Python 3 on that system.
...
prod:
  hosts:
    experimental.example.com:
      ansible_user: root
      host: experimental
      hostname: experimental.example.com
```
...
Deploy the stack defined in `devops/stacks/experimental.example.com.yml` to the remote server with:
